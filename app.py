...
def compute_everything(df_raw: pd.DataFrame, window: int) -> pd.DataFrame:
    ...
    # Selic anualizada a partir do CDI diário: r_aa = (1 + r_dia) ^ 252 - 1
    df["SelicAnualizada"] = (1.0 + df["CDI"]).pow(window) - 1.0  # decimal (ex: 0.135)
    ...
    # CDI anual rolling (fator) para bins e para %CDI histórico
...
def build_cdi_regime_table(df: pd.DataFrame, bins: np.ndarray, z: float) -> pd.DataFrame:
    ...
    for i in range(len(bins) - 1):
        ...
        for name in carteira_order:
            ...
            # %CDI "base" (mantemos por auditoria; as colunas VIS serão consistentes)
            if cdi_mid_pct != 0:
...
def make_grouped_bar_chart(bar_long: pd.DataFrame, y_field: str, y_title: str) -> alt.Chart:
    ...
    bars = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("cdi_range:N", sort=alt.SortField(field="range_order", order="ascending"), title="Faixa do CDI (% a.a.)"),
...
def make_dual_axis_selic_base100_with_cdi(df: pd.DataFrame) -> alt.Chart:
    ...
    base_lines = (
        ...
        .encode(
            ...
            strokeDash=alt.condition(alt.datum.serie == "CDI", alt.value([6, 4]), alt.value([1, 0])),
            tooltip=[
...
def make_historical_annualized_pct_cdi(df: pd.DataFrame) -> alt.Chart:
    """
    Evolução histórica das estratégias ANUALIZADAS em % do CDI:
      %CDI(t) = (Retorno_aa_% / CDI_aa_%) * 100
    onde Retorno_aa_% = (FatorAnual - 1)*100 e CDI_aa_% = (CDIAcumuladoAnual - 1)*100
    """
    tmp = df[["CDIAcumuladoAnual"] + [f"FatorAnual_{n}" for n in carteira_order]].copy()

    cdi_pct = (tmp["CDIAcumuladoAnual"] - 1.0) * 100.0
    cdi_pct = pd.to_numeric(cdi_pct, errors="coerce")
    cdi_pct = cdi_pct.where(cdi_pct.abs() > 1e-9)  # evita div/0

    out = pd.DataFrame(index=tmp.index)
    for n in carteira_order:
        port_pct = (tmp[f"FatorAnual_{n}"] - 1.0) * 100.0
        out[n] = (port_pct / cdi_pct) * 100.0

    long_df = out.reset_index().melt(
        id_vars=[out.reset_index().columns[0]],
        var_name="carteira",
        value_name="pct_cdi"
    )
    long_df = long_df.rename(columns={long_df.columns[0]: "data"})
    long_df["pct_cdi"] = pd.to_numeric(long_df["pct_cdi"], errors="coerce")

    color_scale = alt.Scale(domain=carteira_order, range=[CARTEIRA_COLOR[c] for c in carteira_order])

    return (
        alt.Chart(long_df.dropna(subset=["pct_cdi"]))
        .mark_line()
        .encode(
            x=alt.X("data:T", title="Data"),
            y=alt.Y("pct_cdi:Q", title="Retorno anualizado (% do CDI)"),
            color=alt.Color("carteira:N", scale=color_scale, legend=alt.Legend(title="Carteira")),
            tooltip=[
                alt.Tooltip("data:T", title="Data"),
                alt.Tooltip("carteira:N", title="Carteira"),
                alt.Tooltip("pct_cdi:Q", title="% do CDI", format=".2f"),
            ],
        )
        .properties(height=420, title="Evolução histórica — Estratégias anualizadas (% do CDI)")
    )

# ---------- helpers robustos para interpolação ----------
def _prepare_xy_unique(sub: pd.DataFrame, x_col: str, y_col: str) -> pd.DataFrame:
    ...
    # remove duplicatas em x -> média do y
...
def plot_continuo_plotly_from_bins(
    ...
    # Linha contínua (hover correto)
    custom_line = np.stack([y_low_grid, y_high_grid], axis=1)
    hover_line = (
        carteira
        + "<br>CDI midpoint (% a.a.): %{x:.2f}%"
        + "<br>Média: %{y:.2f}"
        + "<br>Min (Lower " + ic_label + "): %{customdata[0]:.2f}"
        + "<br>Max (Upper " + ic_label + "): %{customdata[1]:.2f}"
        + "<extra></extra>"
    )
    fig.add_trace(go.Scatter(
        x=x_grid, y=y_grid,
    ...
    hover_pts = (
        carteira
        + "<br>CDI Range: %{customdata[0]}"
        + "<br>Obs: %{customdata[1]}"
        + "<br>CDI midpoint (% a.a.): %{x:.2f}%"
        + "<br>Média: %{y:.2f}"
        + "<br>Min (Lower " + ic_label + "): %{customdata[2]:.2f}"
        + "<br>Max (Upper " + ic_label + "): %{customdata[3]:.2f}"
        + "<extra></extra>"
    )
...
def plot_all_three_lines_no_ic(
    ...
    for carteira in carteira_order:
        sub = res_bins[res_bins["Carteira"] == carteira].copy().sort_values("_range_order")
        ...
        try:
            ...
            st.warning(f"Falha ao interpolar {carteira}: {e}")
            continue

        hovertemplate = (
            carteira
            + "<br>CDI midpoint: %{x:.2f}%"
            + "<br>Retorno: %{y:.2f}"
            + "<extra></extra>"
        )
...
# filtro de tendência (somente para tabela/bins/barras)
df_regime = df.copy()
...
# =========================
# ✅ NOVO: HISTÓRICO — ANUALIZADO EM % DO CDI
# =========================
st.divider()
st.subheader("Evolução histórica — Estratégias anualizadas em % do CDI (rolling 252)")
st.altair_chart(make_historical_annualized_pct_cdi(df), use_container_width=True)
...
# barras (também afetadas por Base vs Suavizado)
